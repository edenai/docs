...
- If a snippet creates something the account is rationed on and cannot reclaim (today: `POST /v3/manage/keys/`, where revoking a key leaves its slot spent against the plan's 50-per-user limit), mark it `{/* quota-test: reason */}`. These never run automatically, not even weekly, because the resource does not refill and any cadence eventually wedges the suite; they need `EDEN_AI_RUN_QUOTA_CALLS=1`. Read-only samples on the same endpoints do not need the marker.
- Every generated module is syntax-checked by `tests/test_snippets_compile.py`, including blocks that never execute, so a skipped snippet still cannot contain invalid Python.
