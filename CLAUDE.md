...
- Model names in prose are checked too, not just in snippets: `tests/test_model_references.py` validates every backticked `provider/model` and `feature/subfeature/provider[/model]` against the live catalogue. If it flags an id, fix the doc rather than the test; the failure message lists the live alternatives.
- Exception: the pages under `v3/expert-models/features/` are generated by `scripts/generate_features.py`. Do NOT hand-edit them to satisfy that check — the next generator run reverts it. Regenerate instead, and if the generated output is wrong, fix the generator.
