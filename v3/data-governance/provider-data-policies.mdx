---
sidebarTitle: "Provider Data Policies"
title: "Provider Data Use & Retention"
icon: "user-shield"
...
<TechArticleSchema
  title={"Provider Data Use & Retention"}
  description={"Whether each AI provider trains on your data, how long they keep it, and which ones offer Zero Data Retention (ZDR), for every model available through Eden AI."}
  path="v3/data-governance/provider-data-policies"
...
<Note>
  This information was last updated on 27 August 2026.
</Note>
...
This is a separate question from retention: it is about *where* your data is processed, not whether it is stored. To keep processing inside the EU, use the [EU endpoint](/v3/data-governance/eu-endpoint) (`api.eu.edenai.run`), which routes only through EU-eligible providers: Amazon Bedrock (`eu-west-1` and `eu-north-1`), Google Vertex (EU multi-region), Microsoft Azure (EU Data Zone), Mistral, OVHcloud and Scaleway (EU-hosted), and Databricks, Nebius and Qwen (EU-region routing). Confirm the live list any time with `api.eu.edenai.run/v3/models`.

## Reduce your data exposure

- **Zero Data Retention** (nothing stored): for LLMs, pin your `model` and `fallbacks` to Amazon Bedrock, Cerebras, Cloudflare, Databricks, DeepInfra, Fireworks, Flex AI, Google (Vertex), Groq, Infomaniak, IONOS, Lilac, Microsoft Azure, Mistral, Nebius, OVHcloud, Perplexity, Qwen, Scaleway, TensorX, Together, xAI, or Z.ai. For Expert Models, pick a provider marked **Yes** in the table above.
