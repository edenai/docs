...
We give you the address when we enable the feature for your organization, rather than publishing it here. What you can plan for in advance:

| | Value |
|---|---|
| Format | A single IPv4 address, not a range — one firewall rule covers it |
| Location | Paris, France (AWS `eu-west-3`) |
| Direction | Outbound only — Eden AI to your server |
...
- **Inbound requests.** This is about traffic leaving Eden AI, not reaching it. Your calls to `api.edenai.run` are unaffected, and you do not need to whitelist or allowlist anything to call the API.
...
1. [Contact support](/v3/general/support) — the chat bubble on this page or your dedicated channel — and ask for static IP on your organization.
