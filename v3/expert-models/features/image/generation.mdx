...
The response contains an `output.items` array with one entry per generated image (based on `num_images`).

| Field | Type | Description |
|-------|------|-------------|
| `output.items[].image` | string | Base64-encoded image |
| `output.items[].image_resource_url` | string | Hosted URL of the generated image |


## Available Providers

<Tip>
  Providers and pricing change frequently. For the live list, use the [playground](https://app.edenai.run/playground/universal-ai?feature=image%2Fgeneration) or call `GET https://api.edenai.run/v3/info/image/generation`. OpenAI model prices vary by usage (resolution, quality) — the prices below are estimates.
</Tip>
...
| leonardo (Leonardo Diffusion XL) | `image/generation/leonardo/Leonardo Diffusion XL` | $0.014 per image |
| leonardo (Leonardo Kino XL) | `image/generation/leonardo/Leonardo Kino XL` | $0.017 per image |
| leonardo (Leonardo Lightning XL) | `image/generation/leonardo/Leonardo Lightning XL` | $0.012 per image |
