...
icon: "database"
title: "Caching"
description: "Eden AI automatically caches responses for repeated identical requests. When a request matches a previous one (same model and same input), the cached response is returned at no additional cost."
---
...
<Info>
This page describes **response caching**, which returns an earlier result. For LLM prefix reuse that still generates a fresh response, see [Prompt Caching](/v3/llms/prompt-caching).
</Info>
