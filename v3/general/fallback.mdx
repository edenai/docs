...
- **A provider-less name works in `fallbacks` too.** It resolves to one provider of that model, so `{"model": "openai/gpt-latest", "fallbacks": ["gpt-5.6-sol"]}` is valid.
