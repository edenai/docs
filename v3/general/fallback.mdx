...
|---|---|---|
| **Models you name** | The next entry in your `fallbacks` list — a different model | `fallbacks` |
...
When you name a model **without** a provider — `gpt-oss-120b` rather than `groq/gpt-oss-120b` — Eden AI chooses the provider, and will try a different provider of that same model if the chosen one fails. This happens whether or not you supplied a `fallbacks` list.
...
- **Repeats are kept, not deduplicated.** Listing the same model twice (`["a/m", "b/m", "a/m"]`) means it is genuinely attempted twice. A rate limit or a 5xx does not repeat deterministically, so a deliberate repeat is a retry — the only one Eden AI performs, since it never retries on its own.
- **A provider-less name works in `fallbacks` too.** It resolves to one provider of that model, so `{"model": "openai/gpt-4o", "fallbacks": ["gpt-oss-120b"]}` is valid.
...
"edenai_metadata": {
  "attempt": 2,
  "attempts": [
    {"provider": "cerebras", "model": "cerebras/gpt-oss-120b", "status": 402, "region": "global"},
