...
You can configure default tags on an API key in the dashboard or through the Management API. Every call made with that key automatically inherits those tags, without needing to pass them on each request.
...
### Combining sources and precedence

Tags can come from three sources: the API key's default tags, the `X-EdenAI-Tags` header, and the body `tags` field.

They are merged by key in order of precedence:

1. API key default tags
2. The `X-EdenAI-Tags` header
3. The body `tags` field

Each source wins over the previous on the same key:

| Sources | Recorded |
|---|---|
| Key `client=acme,env=dev`, header `env=prod`, body `{"project": "invoices"}` | `client=acme`, `env=prod`, `project=invoices` |
| Key `client=acme` and body `{"client": "globex"}` | `client=globex` |
...
| | Rule |
|---|---|
| Number | At most 10 tags per call, counted after merging the API key defaults, header, and body |
...
| Duplicates | The same key can appear across sources, with later sources winning. Within a single source, keys are compared in lowercase, so `Client=a,client=b` in one header is a duplicate |
...
| Problem | Message |
|---|---|
| More than 10 tags | `at most 10 tags per call, got 11` (or `... got 11 (3 set on the API key)` when key defaults contribute) |
...
If an API key has an invalid default tag stored, the invalid tag is dropped and logged rather than refusing the call, since the caller cannot fix it on the request.
