...
The header works on every v3 endpoint, including file uploads, [video generation](/v3/llms/video-generation) and collections. Write `key=value` pairs separated by commas:
...
response = litellm.completion(
    # "openai/" selects LiteLLM's OpenAI-compatible client; the rest is the Eden AI model
    model="openai/openai/gpt-latest",
...
For [Claude Code](/v3/integrations/claude-code) and [OpenCode](/v3/integrations/opencode), start from their setup pages; the lines above only add the tags.
