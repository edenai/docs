  ...
  <CodeBlock title="Python">
    ...
    async def analyze_text_parallel(text):
        ...
        analyses = [
            ...
            {"name": "ai_detection", "model": "text/ai_detection/openai/gpt-4"},
    ...
    print("AI Detection:", results["ai_detection"]["is_ai_generated"])
...
<CodeBlocks>
  <CodeBlock title="Python">
    ...
    def smart_content_workflow(text):
        ...
        # Step 1: Check if AI-generated
        ai_detection_payload = {
            "model": "text/ai_detection/openai/gpt-4",
            "input": {"text": text}
        }
        ai_response = requests.post(url, headers=headers, json=ai_detection_payload)
        is_ai = ai_response.json()["output"]["is_ai_generated"]

        workflow_result = {"is_ai": is_ai}

        if is_ai:
            # AI content → Check for plagiarism
            print("AI content detected, checking plagiarism...")

            plagiarism_payload = {
                "model": "text/plagiarism_detection/copyleaks",
                "input": {"text": text}
            }
            plagiarism_response = requests.post(url, headers=headers, json=plagiarism_payload)
            workflow_result["plagiarism"] = plagiarism_response.json()["output"]

        else:
            # Human content → Run sentiment analysis
            print("Human content detected, analyzing sentiment...")
        ...
        workflow_result["moderation"] = moderation_response.json()["output"]
...
<CodeBlocks>
  <CodeBlock title="Python">
    ...
    def process_product_workflow(image_path, description):
        ...
        # 4. Check description for AI generation
        ai_detection_payload = {
            "model": "text/ai_detection/openai/gpt-4",
            "input": {"text": description}
        }
        ai_response = requests.post(url, headers=headers, json=ai_detection_payload)
        is_ai_generated = ai_response.json()["output"]["is_ai_generated"]

        return {
            "image_safe": is_safe,
            "detected_objects": [obj["label"] for obj in detected_objects],
            "description_sentiment": sentiment["sentiment"],
            "description_ai_generated": is_ai_generated,
            "approved": is_safe and sentiment["sentiment"] in ["Positive", "Neutral"]
        }
  ...
  <CodeBlock title="Python">
    ...
    def comprehensive_moderation_workflow(content_type, file_path=None, text=None):
        ...
        if content_type == "text":
            ...
            response = requests.post(url, headers=headers, json=ai_payload)
            moderation_results["ai_detection"] = response.json()["output"]
...
- [Text Features](text-features.mdx) - Text analysis capabilities
- [OCR Features](ocr-features.mdx) - Document processing
- [Image Features](image-features.mdx) - Image analysis
- [Upload Files](../../how-to/upload/upload-files.mdx) - File management
