...
You select a model with the `provider:model_id` syntax. For Eden AI, the provider is `edenai` and the model id follows Eden AI's `provider/model` scheme — for example `edenai:vertex/gemini-flash-latest`.
...
<CodeGroup>
```bash pip
pip install git+https://github.com/andrewyng/aisuite.git openai
```

```bash poetry
poetry add git+https://github.com/andrewyng/aisuite.git openai
```
</CodeGroup>

<Note>
  The Eden AI provider is on aisuite's `main` branch but has not made it into a
  PyPI release yet, so install from the repository. Once aisuite publishes a
  release that includes it, `pip install aisuite openai` is enough.
</Note>
...
response = client.chat.completions.create(
    model="edenai:vertex/gemini-flash-latest",  # edenai:<provider>/<model>
    messages=[
...
- `edenai:anthropic/claude-haiku-latest`

**Google**
- `edenai:vertex/gemini-2.5-pro`
