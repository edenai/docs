...
export ANTHROPIC_MODEL="anthropic/claude-opus-latest"     # default
export ANTHROPIC_MODEL="anthropic/claude-sonnet-latest"   # cheaper, faster
export ANTHROPIC_MODEL="anthropic/claude-haiku-latest"    # fastest
```

```bash Other providers
export ANTHROPIC_MODEL="openai/gpt-latest"
export ANTHROPIC_MODEL="vertex/gemini-2.5-pro"
```
</CodeGroup>
...
Eden AI also hosts an [MCP server](/v3/expert-models/mcp-server) that exposes the expert model catalog (OCR, web search, speech, translation and more) as tools. Add it and Claude Code can read documents or search the web through the same API key:

```bash
claude mcp add --transport http edenai https://mcp.edenai.run/mcp \
  --header "Authorization: Bearer YOUR_EDEN_AI_API_KEY"
```

To share it with your team instead, commit the equivalent block to the project's `.mcp.json`:
...
Keep the `type` field: without it Claude Code reads the entry as a stdio server. Run `/mcp` to confirm the connection. See the [MCP server page](/v3/expert-models/mcp-server) for the full tool catalog and the conventions its tools share.
...
{/* skip-test: reads $ANTHROPIC_API_KEY, which the reader exports earlier on the page */}
```bash
...
curl -X POST https://api.edenai.run/v3/chat/completions \
  ...
  -d '{"model": "anthropic/claude-opus-latest", "messages": [{"role": "user", "content": "ping"}]}'
```

### Model not found

Use the full `provider/model` string (e.g. `anthropic/claude-opus-latest`, not `claude-opus-4-7`). Confirm the ID is in the catalog returned by `GET /v3/models`.
