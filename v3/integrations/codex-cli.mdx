...
Codex reads `EDENAI_API_KEY` from the environment at connect time and sends it as an `Authorization: Bearer` header, which keeps the key out of a config file that may be committed. The CLI equivalent is `codex mcp add edenai --url https://mcp.edenai.run/mcp`.
