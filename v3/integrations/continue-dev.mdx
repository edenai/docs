---
title: "Continue.dev"
icon: "/integration-logo/continuedev-icondoc.svg"
description: "Configure Continue.dev, the open-source AI code assistant, to use Eden AI for access to 500+ models."
---
...
<TechArticleSchema
  title={"Continue.dev"}
  description={"Configure Continue.dev, the open-source AI code assistant, to use Eden AI for access to 500+ models."}
  path="v3/integrations/continue-dev"
  articleSection="Coding Agents"
  about={"AI Coding Assistants"}
  proficiencyLevel="Intermediate"
  keywords={["Eden AI", "AI API", "Continue.dev"]}
  datePublished="2026-05-06T00:00:00Z"
  dateModified="2026-08-31T00:00:00Z"
/>

Configure Continue.dev, the open-source AI code assistant, to use Eden AI for access to 500+ models.
...
- **500+ models**: Access OpenAI, Anthropic, Google, Cohere, and more
...
{/* skip-test: elides the models array with [...] to focus on contextProviders */}
<CodeGroup>
```json config.json
{
  "models": [...],
...
Eden AI also hosts an [MCP server](/v3/expert-models/mcp-server) that exposes the expert model catalog (OCR, web search, speech, translation and more) as tools. Add the server to your assistant's `config.yaml`:
...
The tools become available in Agent mode. See the [MCP server page](/v3/expert-models/mcp-server) for the full tool catalog and the conventions its tools share.
...
2. Ensure `apiBase` is exactly: `https://api.edenai.run/v3`
3. Reload the IDE
...
- **Quick edits**: Use `openai/gpt-mini-latest` for speed
- **Complex reasoning**: Use `anthropic/claude-sonnet-latest`
- **Autocomplete**: Use `openai/gpt-mini-latest` for low latency
