...
<CodeGroup>
{/* skip-test: imports Hermes' own `providers` module, which only exists inside a Hermes plugin directory */}
```python __init__.py
...
Eden AI also hosts an [MCP server](/v3/expert-models/mcp-server) that exposes the expert model catalog (OCR, web search, speech, translation and more) as tools. Add it under `mcp_servers` in Hermes' config file (`hermes config path` prints the location):
...
Hermes interpolates `${VAR}` in server entries, so this reuses the `EDENAI_API_KEY` you already added in [step 2](#2-add-your-api-key). The server's tools register into Hermes' normal tool registry at startup, which means they chain with built-in tools in a single workflow. Run `/reload-mcp` in an active chat to pick up config changes without a restart.
