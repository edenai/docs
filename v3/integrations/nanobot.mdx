...
Eden AI model IDs are `provider/model`, and nanobot forwards the ID unchanged, including the vendor prefix. So the preset above sends `vertex/gemini-flash-latest` upstream as-is.
...
Read the catalogue rather than copying a model list from a guide. Availability and pricing change, and IDs are retired over time. `vertex/gemini-flash-latest` is an alias that always resolves to the current Gemini Flash, which makes it a safe default for a config you do not want to revisit.
