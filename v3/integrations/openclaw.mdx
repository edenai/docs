...
```json ~/.openclaw/openclaw.json
{
  ...
  "agents": {
    "defaults": {
      "model": {
        "primary": "edenai/anthropic/claude-sonnet-latest"
      }
    }
  }
}
...
- **`agents.defaults.model.primary`** makes the Eden AI model OpenClaw's default. It must be `edenai/<model-id>` — the provider key (`edenai`) plus the model's `id` (e.g. `edenai/anthropic/claude-sonnet-latest`).
...
In the **Model and auth** step, select `Eden AI — Claude Sonnet` — the model you defined in Step 1 (it's already your default from the `agents.defaults.model.primary` setting). Accept the defaults for the remaining steps (Workspace, Gateway, Channels, etc.) unless you have specific local-networking or channel needs.
...
{/* skip-test: one key of the config above, not a whole file */}
```json
...
| Model | Best for |
|---|---|
| `anthropic/claude-sonnet-latest` | **Recommended default** — balanced reasoning, reliable tool calling |
| `anthropic/claude-opus-latest` | Maximum capability |
| `anthropic/claude-haiku-latest` | Fast and cheap |
| `openai/gpt-5` | OpenAI's frontier model |
| `vertex/gemini-2.5-pro` | Long context, multimodal |

Browse the full catalog at [app.edenai.run/models](https://app.edenai.run/models).

## Expert Models as Tools (MCP)

Eden AI also hosts an [MCP server](/v3/expert-models/mcp-server) that exposes the expert model catalog (OCR, web search, speech, translation and more) as tools. Add it with the CLI:

```bash
openclaw mcp set edenai '{"url":"https://mcp.edenai.run/mcp","transport":"streamable-http","headers":{"Authorization":"Bearer YOUR_EDEN_AI_API_KEY"}}'
```
...
See the [MCP server page](/v3/expert-models/mcp-server) for the full tool catalog and the conventions its tools share.
...
Ensure the `id` uses the `provider/model` format (e.g. `anthropic/claude-sonnet-latest`). No bare model names.
