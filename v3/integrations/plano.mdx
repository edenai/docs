...
Read the catalogue rather than copying a list from a guide, since IDs are retired over time. `vertex/gemini-flash-latest` is an alias that always resolves to the current Gemini Flash, which makes it a safe default.
