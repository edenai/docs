...
A decision model answers **typed questions about text**. You send some state and the questions you want answered, and you get back a choice, a score, or a probability, each with a confidence, in a single pass.

TypeSafe calls this class of model **System One**, after the fast, intuitive thinking in Kahneman's *Thinking, Fast and Slow*. Their model Jev is the first of them, and the one Eden AI serves today. The endpoint is named `/v3/systemone` so their SDK works against Eden AI by changing only its base URL.
...
```
GET  /v3/systemone/models      List available decision models
POST /v3/systemone             Ask questions about some state
```
...
response = requests.get("https://api.edenai.run/v3/systemone/models")
print(response.json())
```

```typescript TypeScript
const response = await fetch("https://api.edenai.run/v3/systemone/models");
console.log(await response.json());
```

```bash cURL
curl https://api.edenai.run/v3/systemone/models
```
</CodeGroup>
...
response = requests.post(
    "https://api.edenai.run/v3/systemone",
...
```typescript TypeScript
const response = await fetch("https://api.edenai.run/v3/systemone", {
  method: "POST",
  headers: {
...
```bash cURL
curl -X POST https://api.edenai.run/v3/systemone \
  ...
  -d '{
    ...
    "questions": {
      "is_billing": { "type": "noul", "instructions": "Is this message about billing?" }
    }
  }'
```
</CodeGroup>
...
| Status | Meaning |
| --- | --- |
| 400 | Unknown model. Check `GET /v3/systemone/models` for what is callable. |
...
- **Use `confidence` as a routing signal.** Send low-confidence decisions to a human or to a bigger model rather than treating every answer as final.
