...
{/* skip-test: MODEL_ID is a placeholder the reader fills from GET /v3/embeddings/models */}
```bash cURL
...
corpus = [
    ...
    "Provider routing picks the provider serving a model named without one.",
...
| `user` | string | no | Accepted for OpenAI compatibility. Your value is not forwarded to the provider. |
| `metadata` | object | no | Accepted, but not sent to the provider and not a tag. To split your usage per client or project, use [tags](/v3/general/tags). |
...
{/* skip-test: a payload fragment, model_id comes from the sample above it */}
```python Python
payload = {
    "model": model_id,
...
- **Always pass a concrete `provider/model`.** There is no automatic model selection on embeddings.
