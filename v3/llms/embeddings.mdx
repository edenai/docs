...
| `dimensions` | integer | no | Truncate the output vector. Only supported by models that advertise it (e.g. OpenAI's `text-embedding-3-small` and `text-embedding-3-large`). |
...
- **Dimensions** — some models (e.g. OpenAI's `text-embedding-3-small` and `text-embedding-3-large`) support a `dimensions` parameter to truncate outputs. Smaller vectors save storage and speed up similarity search at a small recall cost.
