...
<Warning>
Provider-hosted `url`s are short-lived. If you need long retention, download the bytes yourself, or use the `/v3/universal-ai` endpoint, which stores results in Eden's S3 and returns a long-lived URL.
</Warning>
