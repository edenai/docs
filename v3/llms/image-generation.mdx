...
<Info>
This is different from [Expert Model image generation](/v3/expert-models/features/image/generation), which wraps Eden's feature API and uploads results to long-lived storage. Use the `/v3/images` endpoints when you want an OpenAI-compatible, drop-in image surface.
</Info>
...
<Warning>
Provider-hosted `url`s are short-lived. If you need long retention, download the bytes yourself, or use [`/v3/universal-ai`](/v3/how-to/universal-ai/text-features) which stores results in Eden's S3 and returns a long-lived URL.
</Warning>
...
- **`file_id` lookups resolve to a presigned URL.** Providers that require raw bytes (currently `openai/gpt-image-*`) will return a 400 — pass a `data:` URL for those.

## Next Steps

<CardGroup cols={2}>
  <Card title="Expert Image Generation" icon="palette" href="/v3/expert-models/features/image/generation">
    Use Eden's feature API for DALL-E, Stable Diffusion, and long-lived storage
  </Card>
