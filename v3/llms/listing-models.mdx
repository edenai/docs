...
The `/v3/models` catalog is deduplicated: when a provider exposes both a stable alias and the dated snapshots it resolves to, the list keeps the canonical stable name (falling back to the newest snapshot when no clean alias exists).
