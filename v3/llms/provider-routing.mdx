...
Which regions a model offers varies by model. Requesting one it is not served from returns a `400` error (model not found or inactive) rather than quietly serving it elsewhere. See [Servers Location](/v3/data-governance/servers-location).
