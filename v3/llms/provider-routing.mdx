...
`sort` never changes **which model** answers, only who serves it. To let Eden AI trade quality for a cheaper model, see [`routing.quality_cost`](/v3/llms/smart-routing#trading-quality-for-cost).
...
After Eden AI observes a cache read, it keeps the conversation on that provider for **10 minutes by default**. Each later cache read refreshes the window.
...
If a request declares more than one duration, Eden AI uses the longest one, capped at the maximum retention of the provider that served the cache read. Providers without a known extended-retention policy use a conservative one-hour ceiling.
