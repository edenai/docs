...
| `auto` | What the `@edenai` router chose, or `null` when you named the model yourself. See below |
...
| `auto` | You used `@edenai`, so Eden AI picked the model too. See [Smart Routing](/v3/llms/smart-routing) |

### When Eden AI chose the model

Name a model and `auto` is `null`, which is what tells a parser that the choice was yours. Send `@edenai` and it reports how the model was chosen:
...
`selection` says how the model was arrived at: `ranked` (the candidates were ranked for this request), `single` (only one was eligible) or `default` (they could not be ranked, so a default was applied). `chosen` is the model handed to the pipeline, and `dropped` lists the `router_candidates` that could not be ranked. [Smart Routing](/v3/llms/smart-routing#seeing-what-was-chosen) documents every field.
...
- **Seeing what `@edenai` chose.** `auto.chosen` names the model, and `auto.selection` says whether it was ranked for this request or applied as a default.
