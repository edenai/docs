...
| `tags` | The [tags](/v3/general/tags) recorded for this call, or `{}` when it has none |
...
`tags` lists the [tags](/v3/general/tags) recorded for the call, merged from the API key's default tags, the `X-EdenAI-Tags` header, and the `tags` body field, with keys in lowercase. It is `{}` when the call has none.
...
- **Checking your tags.** `tags` shows exactly what was recorded for your usage breakdown, after the API key defaults, header, and body are merged.
