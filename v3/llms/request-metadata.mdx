...
| `/v3/universal-ai` | Yes, synchronous calls only, with a shorter block (below) |
...
| `/v3/videos`, `/v3/universal-ai/async` | No |
...
The other fields mean the same as above. A failed attempt reports the provider's own status, even though Universal AI always answers HTTP 200 with `"status": "fail"`.
