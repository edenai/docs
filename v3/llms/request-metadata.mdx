...
`tags` lists the [tags](/v3/general/tags) recorded for the call, from the `X-EdenAI-Tags` header and the `tags` body field, merged, with keys in lowercase. It is `{}` when the call has none.
...
- **Checking your tags.** `tags` shows exactly what was recorded for your usage breakdown, after the header and body are merged.
