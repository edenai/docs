...
icon: "message"
title: "Responses"
description: "Eden AI supports the OpenAI Responses API — a stateful alternative to chat completions that stores conversation history server-side, so you don't need to resend the full message history on each turn."
---
...
|-----------|------|---------|-------------|
| `model` | string | Required | Model string, e.g. `openai/gpt-latest` |
...
{/* skip-test: resp_abc123def456 is a placeholder, the API rejects it as an unknown response id */}
<CodeGroup>
```python Python
import requests
