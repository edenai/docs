...
{/* skip-test: generating a video runs for minutes, past the per-test timeout */}
<CodeGroup>
```python OpenAI SDK
...
{/* skip-test: continues the sample above, whose client it reuses, and posts a local keyframe.png */}
<CodeGroup>
...
{/* skip-test: input_reference needs an image URL a provider can actually fetch */}
<CodeGroup>
```bash cURL (JSON)
