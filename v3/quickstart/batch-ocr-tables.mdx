...
{/* skip-test: the last step polls YOUR_PUBLIC_ID, the job id the step above prints */}
```bash cURL
# 1. Upload the document
...
{/* skip-test: polls three async OCR jobs to completion, which outlasts the per-test timeout */}
<CodeGroup>
```python Python
import csv
import time
