...
# Start the CSV with a header row so read.csv() reads columns, not data.
echo "source_file,page,table,cells" > tables.csv
...
for doc in invoice1.pdf invoice2.pdf invoice3.pdf; do
  ...
  # Flatten cells -> CSV rows tagged with source file, page and table
  echo "$body" | jq -r --arg src "$doc" '
    .output.pages | to_entries[]? as $p | $p.value.tables | to_entries[]? as $t |
    $t.value.rows[]? |
    [$src, ($p.key + 1), ($t.key + 1), ([.cells[]? | .text])[]] | @csv
  ' >> tables.csv
done
